import React from 'react';

const WatVindJeWaarPage = () => {
  return (
    <div className="WatVindJeWaarPage">
      <h1>Wat vind je waar?</h1>
      <h2>Klasgroepen</h2>
      <p>
        Dit zijn de groepen waaraan je gekoppeld staat als leraar of als assistent. Klik op een
        klasgroep om de <b>klaslijsten</b> te bekijken, als je de leraar bent kan je ook de{' '}
        <b>aanwezigheden</b> noteren of <b>feedback</b> geven over je leerlingen.
      </p>
      <p>
        Klik op de locatie van elke groep om meer te weten over de verzamelplaats die gecommuniceerd
        wordt aan de ouders.
      </p>
      <p>
        Alles hangt af van de effectieve inschrijvingen, deze zijn &apos;live&apos; te zien op de
        klaslijsten. Bij groepen onder de 7 leerlingen bestaat er een kans dat de groep geannuleerd
        wordt. Inschrijvingen lopen door tot (net na) het start van de lesreeks, dus niet direct
        panikeren
      </p>
      <h2>Lesdata & Lesmateriaal</h2>
      <p>
        Bekijk bij lesdata je lessen en klik door op een les om de <b>details</b> ervan te zien. Kies
        tussen de <b>4 view modi</b> en <b>download</b> de lescontent.
      </p>
      <h2>Profiel</h2>
      <p>
        Bekijk hier jouw persoonlijke gegevens en hou deze gegevens up-to-date. Download je
        CodeFever contract(en).
      </p>
      <h2>Opmerkingen over een leerling</h2>
      <p>Op je klaslijst vind je ook:</p>
      <ul>
        <li>informatie die ouders meegeven over hun kind</li>
        <li>én feedback die de vorige leraar heeft ingevuld. </li>
      </ul>
      <p>
        Neem dit goed door. Dit is interessante inside info die je zeker kan helpen bij het
        lesgeven. Bv: gescheiden ouders, papa gestorven, allergieën, labeltjes, ...
      </p>
      <p>
        Spreek zeker ouders aan tijdens de eerste lessen bij wie je denkt dat dit nuttig is. Ouders
        appreciëren dit ten zeerste en mogelijke tips van ouders kunnen je zeker goed helpen in je
        les.
      </p>
      <p>
        Bv: vraag tips aan de ouders van kinderen met ASS zoals &quot;wat moet ik weten over je kind
        zodat je kind zich goed voelt in mijn les en het beste functioneert in mijn les.&quot;{' '}
      </p>
    </div>
  );
};

export default WatVindJeWaarPage;
